...
func _call():
	...
	var call_tile: Vector2 = Vector2(tile_map.local_to_map(global_position).x + x, tile_map.local_to_map(global_position).y + y)
	...
	while tile_map.local_to_map(call_tile) == tile_map.local_to_map(player.global_position) || calling == false:
		...
		if x == 0 && y == 0:
			...
			continue
		#prints(x, y)
		call_tile = Vector2(tile_map.local_to_map(global_position).x + x, tile_map.local_to_map(global_position).y + y)
		#print(call_tile)
		for enemy in enemies:
			if enemy == self:
				continue
